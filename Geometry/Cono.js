class Cono {
    /**
     */
    constructor(gl,radius=1, height=1, Nu=1, Nv=3, material=new FlatMaterial(gl), transform=identity()) {
    ...
    getUVCoordinates(vertices, isFlat) {
        ...
        if (!isFlat) {
            ...
            for (let i=0; i<vertices.length/3; i+=3) {
                ...
                u1 = 0.5 + (Math.atan2(p1.x/this.r, p1.z/this.r) / PI2);
                v1 = 0.5 + (p1.y / this.h);

                p2 = new Vector3(vertices[(i+1)*3], vertices[(i+1)*3 +1], vertices[(i+1)*3 +2]);
                u2 = 0.5 + (Math.atan2(p2.x/this.r, p2.z/this.r) / PI2);
                v2 = 0.5 + (p2.y / this.h);

                p3 = new Vector3(vertices[(i+2)*3], vertices[(i+2)*3 +1], vertices[(i+2)*3 +2]);
                u3 = 0.5 + (Math.atan2(p3.x/this.r, p3.z/this.r) / PI2);
                v3 = 0.5 + (p3.y / this.h);
